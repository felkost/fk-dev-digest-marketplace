...
function load(label: string): RepeatFile {
  ...
  if (!existsSync(file)) {
    console.error(
      `No repeat run for '${label}'. Run: pnpm eval:repeat <pattern> -n <N> --label ${label}`,
    );
    process.exit(1);
  }
...
function rateRow(indent: string, label: string, a?: Series, b?: Series): void {
  ...
  console.log(
    `${indent}${fmtRate(pa)}% -> ${fmtRate(pb)}%  ${col}Δ ${dStr.padStart(4)}${RESET}  ${label}`,
  );
}
...
function metricRow(label: string, a: Stats, b: Stats): void {
  ...
  const sign = d > 0 ? "+" : "";
  console.log(
    `      ${label}: ${a.mean.toFixed(0)} -> ${b.mean.toFixed(0)}  ${col}(${sign}${d.toFixed(0)})${RESET}`,
  );
}
...
function main(): void {
  ...
  for (const id of nodeids) {
    ...
    const practiceTexts = [
      ...new Set([...Object.keys(ta?.practices ?? {}), ...Object.keys(tb?.practices ?? {})]),
    ];
