...
function printTest(agg: NodeAggregate, times: number): void {
  ...
  if (practices.length) {
    for (const [text, s] of practices) {
      console.log(
        `      ${rateColor(s.rate)}${s.passed}/${s.total} ${pct(s.rate).padStart(4)}${RESET}  ${text}`,
      );
    }
  }
...
async function main(): Promise<void> {
  ...
  if (vitestArgs.length === 0 || !Number.isFinite(times) || times < 1) {
    console.error(
      "usage: pnpm eval:repeat <vitest pattern> [-n times<=2] [-t testNamePattern] [--label name]",
    );
    process.exit(1);
  }
  if (times > MAX_TIMES) {
  ...
  console.log(
    `  ${nCases ?? "?"} test case(s) × ${times} runs  (full traces in results/outputs/)\n`,
  );
  ...
  if (label) {
    ...
    writeFileSync(
      file,
      JSON.stringify(
        { label, git_sha: git.sha, dirty: git.dirty, times, vitestArgs, tests },
        null,
        2,
      ),
    );
