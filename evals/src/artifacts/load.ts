...
import { resolveSkillDir, resolveAgentFile } from "./paths.js";
...
export function skillContent(skillName: string): string {
  ...
  if (!dir) throw new Error(`skill not found in any plugin: ${skillName}`);
...
export function agentContent(agentName: string): string {
  const f = resolveAgentFile(agentName);
  if (!f) throw new Error(`agent not found in any plugin: ${agentName}`);
