...
export const runSkillCases = (skill: string, cases: SkillCase[]) =>
  runQualityCases(skill, cases, skillTask);
  ...
  for (const c of cases) {
    test(c.name, async () => {
      if (c.kind === "dispatch") {
        ...
        logTrace(c.name, result);
        try {
          expect(result.subagents, `subagents: ${result.subagents.join(", ")}`).toContain(
            c.expectSubagent,
          );
        } finally {
        ...
        const treatment = await workflowTask(c.prompt, {
          allowedTools: tools,
          maxTurns: c.maxTurns,
        });
