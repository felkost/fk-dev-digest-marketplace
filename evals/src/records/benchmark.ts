...
async function runConfig(config: string, times: number, vitestArgs: string[]): Promise<void> {
  ...
  for (let i = 1; i <= times; i++) {
    ...
    console.log(
      `  ${label}  ${mark} ${passed}/${fresh.length} cases` +
        (meanScore !== null ? `  score ${meanScore}%` : ""),
    );
  }
}
...
const cell = (s: Stats) =>
  `${s.mean.toFixed(0)} ± ${s.stddev.toFixed(0)} [${s.min}–${s.max}] (n=${s.n})`;
...
async function main(): Promise<void> {
  ...
  console.log(
    `  ${GREEN}candidate${RESET} = artifact injected (with skill)   ${DIM}·${RESET}   ${YELLOW}baseline${RESET} = raw model, no artifact\n`,
  );
  ...
  for (const id of nodeids) {
    ...
    const practiceTexts = [
      ...new Set([...Object.keys(c?.practices ?? {}), ...Object.keys(b?.practices ?? {})]),
    ];
  ...
  const report = {
    ...
    tests: nodeids.reduce<Record<string, { candidate?: NodeAggregate; baseline?: NodeAggregate }>>(
      (acc, id) => {
  ...
  console.log(
    `  ${GREEN}candidate${RESET} = with artifact   ${YELLOW}baseline${RESET} = raw model   Δ = candidate − baseline\n`,
  );
...
function renderMarkdown(
  ...
  const rateCell = (s?: Series) =>
    s && s.total ? `${Math.round(s.rate * 100)}% (${s.passed}/${s.total})` : "—";
  const lines: string[] = [];
  ...
  lines.push("|---|---|---|");
  lines.push(
    `| pass_rate | ${pct(r.run_summary.candidate.pass_rate)} | ${pct(r.run_summary.baseline.pass_rate)} |`,
  );
  lines.push(
    `| tokens_out | ${cell(r.run_summary.candidate.outputTokens)} | ${cell(r.run_summary.baseline.outputTokens)} |`,
  );
  lines.push(
    `| duration_ms | ${cell(r.run_summary.candidate.durationMs)} | ${cell(r.run_summary.baseline.durationMs)} |`,
  );
  lines.push(
    `| num_turns | ${cell(r.run_summary.candidate.numTurns)} | ${cell(r.run_summary.baseline.numTurns)} |`,
  );
  lines.push("");
  ...
  for (const id of nodeids) {
    ...
    lines.push(
      `| **${short}** | ${rateCell(candAgg[id]?.pass)} | ${rateCell(baseAgg[id]?.pass)} |`,
    );
    const texts = [
      ...new Set([
        ...Object.keys(candAgg[id]?.practices ?? {}),
        ...Object.keys(baseAgg[id]?.practices ?? {}),
      ]),
    ];
    for (const t of texts) {
      lines.push(
        `| ${t} | ${rateCell(candAgg[id]?.practices[t])} | ${rateCell(baseAgg[id]?.practices[t])} |`,
      );
    }
  }
  lines.push("");
  ...
  for (const f of r.flags as { nodeid: string; practice?: string; flags: Flag[] }[]) {
    const where = f.practice
      ? `${f.nodeid.split(" > ").slice(-1)[0]} — ${f.practice}`
      : f.nodeid.split(" > ").slice(-1)[0];
