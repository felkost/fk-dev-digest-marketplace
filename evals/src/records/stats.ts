...
const series = (passed: number, total: number): Series => ({
  passed,
  total,
...
export type Flag =
  "non_discriminating" | "always_failing" | "flaky" | "cost_regression" | "missing_data";
...
export function computeFlags(
  ...
  for (const r of [cr, br]) {
    if (r !== undefined && r > FLAKY_LOW && r < FLAKY_HIGH && !flags.includes("flaky"))
      flags.push("flaky");
  }
  const { candTokens, baseTokens } = opts;
  if (candTokens && baseTokens && candTokens > baseTokens * COST_REGRESSION_RATIO)
    flags.push("cost_regression");
