...
const series = (passed: number, total: number) => ({
  passed,
  total,
...
describe("computeFlags", () => {
  ...
  test("cost_regression when candidate tokens exceed 125% of baseline", () => {
    expect(
      computeFlags(series(5, 5), series(5, 5), { candTokens: 130, baseTokens: 100 }),
    ).toContain("cost_regression");
    expect(
      computeFlags(series(5, 5), series(5, 5), { candTokens: 120, baseTokens: 100 }),
    ).not.toContain("cost_regression");
  });
});
