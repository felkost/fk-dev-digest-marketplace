...
| `groundFindings()` silently drops findings where `finding.file` is `null`; LLM sometimes omits `file` for repo-level comments. Always validate finding count after grounding. `core-lib/src/grounding.ts:112` | **Pass** | Names the function, the condition, the consequence, the fix |
