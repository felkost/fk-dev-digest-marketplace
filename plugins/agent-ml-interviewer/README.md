...
Codex/ChatGPT-агентів); **11 із 21 скіла** мають `scripts/` — детерміновані
...
У цьому репо вже підключено: `.claude/skills/<name>` — **junction-и** на кореневі
теки (єдине джерело правди, без копій). Для інших проєктів — скопіювати теки
скілів у `~/.claude/skills/` або `<проєкт>/.claude/skills/`.
...
Збирає `dist/ml_advisor_knowledge.zip` + перевіряє 8000-байтовий ліміт
...
```bash
python tests/smoke_test.py      # структура, junction-и, самотести, байти GPT
```
