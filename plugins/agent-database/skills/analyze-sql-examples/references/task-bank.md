...
## 9. ClickHouse на 300 млн подій/добу: `Too many parts`, повільний звіт і дублікати — одна хвороба
**Умова:** таблиця кліків ~2 млрд рядків, +300 млн/добу, ретеншн 3 роки. `ReplacingMergeTree(event_time)`, `PARTITION BY toDate(event_time)`, `ORDER BY (user_id, event_time)`, `revenue Nullable(Float64)`, `country`/`event_type` як `String`. У проді: 200 продюсерів шлють події **по одному рядку** через HTTP; щоночі `OPTIMIZE TABLE events FINAL` «щоб прибрати дублікати»; правки — через `ALTER TABLE … UPDATE`; звіт «кліки й виручка по країнах за 7 днів» повільний; раз на кілька днів вставки падають з `Too many parts`. Знайти причини й перепроєктувати.
**Середовище/діалект:** ClickHouse 24.8+/25.x, self-hosted, **нереплікована** таблиця. Припущення: продюсери at-least-once; `country` ~250 значень.
**Аналіз:** 300 млн/добу ≈ **3 472 події/с**; «рядок = INSERT» → 3 472 парти/с проти орієнтиру ~1 `INSERT`/с — перевищення в ~3 400× і пряма причина `Too many parts`. Звіт за 7 днів = 2,1 млрд рядків ≈ **вся таблиця**, а `ORDER BY` починається з високо-кардинального `user_id`, тож розріджений індекс для фільтра за часом не працює. Ключ дедуплікації = `ORDER BY (user_id, event_time)` → два різні кліки в одну мілісекунду **склеюються** (втрата даних), а колонка версії `event_time` **входить** в `ORDER BY` → всередині групи стала → «максимальна версія» вироджується. `OPTIMIZE FINAL` ігнорує запобіжник ~150 GB (`max_bytes_to_merge_at_max_space_in_pool`): роздутий парт випадає з фонових злиттів назавжди, тож **дублікати накопичуються саме через нічну чистку**. Мутації переписують цілі парти, незворотні, конкурують за той самий пул. 3 роки × 300 млн ≈ 328 млрд рядків — система спроєктована в ~164× менше за цільовий обсяг.
**Розв'язок:**
```sql
CREATE TABLE events_v2 (
    event_time  DateTime64(3, 'UTC') CODEC(DoubleDelta, ZSTD(1)),
    event_date  Date MATERIALIZED toDate(event_time),
    event_id    UUID,                              -- ключ ідемпотентності від продюсера
    ingested_at DateTime DEFAULT now(),            -- версія ПОЗА ORDER BY
    user_id     String, session_id String,
    country     LowCardinality(String), event_type LowCardinality(String),
    url         String CODEC(ZSTD(3)),
    revenue     Decimal(18, 4) DEFAULT 0           -- гроші не Float64
)
ENGINE = ReplacingMergeTree(ingested_at)
PARTITION BY toYYYYMM(event_time)                  -- 36 партицій замість 1095
ORDER BY (user_id, event_time, event_id)
TTL event_date + INTERVAL 3 YEAR DELETE;

CREATE TABLE events_country_daily (
    event_date Date, country LowCardinality(String), event_type LowCardinality(String),
    clicks UInt64, revenue Decimal(38, 4)
) ENGINE = SummingMergeTree
PARTITION BY toYYYYMM(event_date) ORDER BY (event_date, country, event_type);

CREATE MATERIALIZED VIEW mv_events_country_daily TO events_country_daily AS
SELECT toDate(event_time) AS event_date, country, event_type,
       count() AS clicks, sum(revenue) AS revenue
FROM events_v2 GROUP BY event_date, country, event_type;

SELECT country, sum(clicks) AS clicks, sum(revenue) AS revenue
FROM events_country_daily
WHERE event_date >= today() - 7 AND event_date < today()   -- напівінтервал
GROUP BY country ORDER BY revenue DESC;

SET async_insert = 1, wait_for_async_insert = 1, async_insert_busy_timeout_ms = 1000;
```
Правки — новим рядком із тим самим `event_id` і свіжим `ingested_at`; масові — `ALTER TABLE … REPLACE PARTITION … FROM staging`. Нічний `OPTIMIZE FINAL` прибрати повністю.
**Пояснення:** `event_id` в кінці `ORDER BY` робить ключ дедуплікації рівним ідентифікатору події — ретраї склеюються, різні кліки більше не з'їдають один одного; висока кардинальність в кінці ключа коштує мало. `ingested_at` винесено **поза** `ORDER BY`, тому «максимальна версія» нарешті визначена. Партиціювання за часом **події** утримує ретрай у тій самій партиції (злиття не перетинають межі партицій). `async_insert` із таймаутом 1000 мс складає 3 472 події/с приблизно в **один парт на секунду по ~3 500 рядків** — у бюджеті. Роллап зводить звіт із 2,1 млрд рядків до ~35 тис.
**Ризики:** `async_insert` **вимикає дедуплікацію вставок**, а на нереплікованому `MergeTree` вона й так вимкнена за замовчуванням → без `event_id` дублікати неминучі. **MV рахує рядок на вставці, тобто ДО дедуплікації** — вітрина завищиться назавжди, навіть коли сира таблиця стане чистою; дедуплікувати треба до ClickHouse або перебудовувати вітрину. `Nullable(Float64)` → `Decimal DEFAULT 0` не змінює `SUM`, але **змінює `AVG`/`COUNT`** (знаменник стрибає з «кліків із покупкою» на «всі кліки»). Виправлення сирих даних не доходять до вітрини автоматично. Міграція 2 млрд рядків — не онлайн-операція. `ORDER BY` наявної таблиці не змінюється (`MODIFY ORDER BY` лише дописує стовпці й **не пересортовує**) → потрібна нова таблиця.
**Альтернативи:** **Kafka engine + MV** замість HTTP-інджесту — правильна відповідь на «багато дрібних продюсерів» (черга бере буферизацію, ретраї, backpressure). `Buffer`-таблиця — простіше за async, але дані **втрачаються при падінні сервера**. `PROJECTION` замість другого порядку рядків (не ланцюжаться, блокують lightweight delete). `AggregatingMergeTree` + `uniqState`/`uniqMerge`, якщо звіт зажадає унікальних користувачів — `SummingMergeTree` цього не вміє. Словник + `dictGet` замість `JOIN` на довідники. PostgreSQL/MySQL на цьому профілі — **не альтернатива** (328 млрд рядків); їхня роль тут — довідники, які ClickHouse тягне словником.
**Обмеження:** реплікація й шардування поза розбором (на 328 млрд рядків один вузол — окрема розмова); числа — арифметика з умови, не заміри на залізі (`EXPLAIN indexes = 1`, `send_logs_level='trace'`); чи використовує наявний звіт `FINAL` — невідомо, а від цього залежить, завищений він зараз чи просто повільний; `event_id` треба ще навчитися генерувати на боці продюсера (хеш від полів не годиться — два справді різні кліки дадуть однаковий).
**Чого вчить:** ключ дедуплікації `ReplacingMergeTree` = `ORDER BY` і його партиційна межа; вироджена колонка версії; `OPTIMIZE FINAL` як **причина**, а не ліки; бюджет вставок і арифметика `Too many parts`; роллап через MV замість сканування фактів; MV як джерело подвійного рахунку — той самий механізм, що fan-out (§6a каталогу); зміна метрик при знятті `Nullable` (§3); напівінтервал у фільтрі (§5).

## 10. Бронювання готелів на Cassandra: розрахунок партицій і межі SAI
**Умова:** бекенд бронювання на Cassandra 5.0, чотири запити: Q1 — готелі поблизу POI; Q2 — доступні номери готелю за діапазон дат; Q3 — бронювання гостя за email; Q4 — перегляди сторінки готелю за період. Обсяги: 500 номерів на готель, доступність на 2 роки вперед; популярний готель — до 50 тис. переглядів/добу, історія 1 рік. Колега пропонує **одну** таблицю `hotels` + вторинні індекси на `poi` та `email` + `ALLOW FILTERING` там, де не вистачить, аргументуючи: «у 5.0 є SAI, вторинні індекси більше не проблема». Оцінити й запропонувати модель.
**Середовище/діалект:** Apache Cassandra 5.0, CQL. Припущення: склад полів події перегляду (`user_id`, `session_id`, `referrer`, `device`) ≈ 111 б/рядок; `hotel_id` — короткий текстовий код.
**Аналіз:** в умові є **неоднозначність**, яка змінює архітектуру: «поблизу POI» — це (а) готелі, прив'язані до POI зі списку (виражається природно) чи (б) радіус N км від координат (**у CQL невиразне**, геоіндексу немає). Далі — тлумачення (а), (б) винесено в обмеження. Пропозиція колеги неробоча по всіх пунктах: одна таблиця не вміщає Q2 і Q4 (інші сутності, інший життєвий цикл); зв'язок «готель↔POI» — many-to-many, а не скалярний стовпець; індекс на `email` — висока кардинальність, класичний антипатерн (повертає 1 рядок ціною опитування всіх вузлів); `ALLOW FILTERING` — діагноз, не фікс. **Про SAI чесно:** колега має рацію, що SAI кращий за `2i` і справді у 5.0 — але SAI лишається **локальним** індексом (per-SSTable/per-Memtable), тож запит без partition key однаково йде scatter-gather по кільцю; і головне — SAI **не адресує проблему цієї задачі**, якою є розмір партиції й ретеншн. Індекс не зменшить партицію на 2,6 GB.
**Розв'язок:** таблиця під запит; `hotels_by_poi ((poi_name), hotel_id)`, `available_rooms_by_hotel_date ((hotel_id), date, room_number)`, `reservations_by_guest_email ((guest_email), start_date, confirmation_number)`, і для Q4 — дві таблиці:
```sql
-- Q4 «перегляди за період» = агрегат по добах
CREATE TABLE page_views_by_hotel_day (
  hotel_id text, year int, day date, views counter,
  PRIMARY KEY ((hotel_id, year), day)
) WITH CLUSTERING ORDER BY (day ASC);

-- лише якщо треба drill-down по окремих подіях
CREATE TABLE page_views_by_hotel_week (
  hotel_id text, year_week int, view_ts timestamp, view_id timeuuid,
  user_id uuid, session_id uuid, referrer text, device text,
  PRIMARY KEY ((hotel_id, year_week), view_ts, view_id)
) WITH CLUSTERING ORDER BY (view_ts DESC, view_id ASC)
  AND default_time_to_live = 31536000;
```
**Пояснення (розрахунок — ядро задачі):** `Nv = Nr(Nc − Npk − Ns) + Ns`, метадані ≈ 8 б/комірку, ціль < 100 MB, жорсткий ліміт 2 млрд комірок.
**Q2:** `Nr` = 500 × 730 = 365 000; `Nc`=4, `Npk`=3 → `Nv` = 365 000 комірок (0,018 % ліміту). Диск: 5 + 365 000×7 + 365 000×8 ≈ **5,5 MB** → **бакетувати не треба**. Зверни увагу: 365 тис. рядків утричі перевищують поширене емпіричне «до 100 тис. рядків на партицію» — це правило є лише **дешевим проксі** для 100 MB, і за конфлікту з прямим розрахунком вір байтам.
**Q4 сирі події без бакета:** `Nr` = 50 000 × 365 = 18,25 млн; `Nc`=7, `Npk`=3 → `Nv` = 73 млн комірок = 3,65 % ліміту — **«проходить»**. А диск: 18,25 млн × 111 + 73 млн × 8 ≈ **2,61 GB**, у 26× над ціллю. Ось де пастка: ліміт комірок майже ніколи не є вузьким місцем, байти — є. Підбір бакета числом: рік 2610 MB ✗, місяць 214 MB ✗ (удвічі над ціллю, попри «типовий компроміс»), **тиждень ≈50 MB ✓**, доба 7,2 MB ✓ за розміром, але 365 партицій на річний запит. Беремо тиждень.
**Q4 агрегат:** `Nr`=365, `Nv`=365 комірок, ≈ **7,3 KB** — за розміром бакет не потрібен, але `year` лишаємо, **бо `counter` не підтримує TTL**: ретеншн можливий лише видаленням цілих партицій.
**Ризики:** гонитва за останнім номером — без LWT два бронювання перезапишуть одне одного (upsert, last-write-wins) **без помилки**; LWT ~4 круговерті, для бронювання прийнятно, для гарячого шляху ні (загальних транзакцій у 5.0 немає, Accord очікується в 5.1). Мультипартиційний батч на запис бронювання дає атомарність **без ізоляції**. `counter` неідемпотентний — ретрай після таймауту може порахувати перегляд двічі. `NULL`, прибінджений в опційні поля форми, — лавина tombstone (використовуй `unset`). `guest_email` як partition key: змінив email — бронювання «зникли».
**Альтернативи:** **Q3 через `guest_id`** + маленька `guest_by_email(email PK, guest_id)` — 2 походи замість 1, зате email стає змінним атрибутом, а не адресою даних (**рекомендований варіант для проду**). **SAI там, де доречний** — вторинний фільтр **усередині** партиції (`WHERE hotel_id = ? AND date >= ? AND is_available = true`). **MV** замість ручної денормалізації — знімає ризик розїзду копій ціною запису й tombstone. **Q4 повністю поза Cassandra:** якщо «аналітика популярності» означає довільні зрізи — це події → Kafka → **ClickHouse**, а в Cassandra лишити лічильник для показу на сторінці. **Q1 як геопошук** (тлумачення (б)): geohash-префікс у partition key із дозапитом сусідніх клітин, або зовнішній пошуковий рушій.
**Обмеження:** Q1 у цій моделі — «готелі при POI», не радіус (потребує підтвердження замовника — найважливіше обмеження, і воно з умови, а не з СУБД); 111 б/рядок — припущення, додайте `user_agent` цілком і тижнева партиція вилетить за 100 MB; оцінка не враховує компресію, тобто **консервативна**; ретеншн «2 роки вперед» у Q2 не самоочищається; поза розбором — записуючі шляхи для персоналу готелю, скасування, овербукінг, мультирегіональна топологія.
**Чого вчить:** п'ятикрокова методологія «спершу запити»; формули `Nv`/`St` з реальною підстановкою; **три таблиці — три різні висновки** (не бакетувати / бакетувати тижнем / бакетувати заради ретеншну), що знімає і «ніколи не бакетуй», і «бакетуй завжди»; чому ліміт комірок проходить там, де байти вже провалено; локальна природа SAI й ціна scatter-gather; LWT як межа атомарності та її вплив на вибір partition key; `counter` без TTL; межа застосовності Cassandra для аналітики (→ ClickHouse). Єдиний у банку кейс із моделюванням Cassandra.
