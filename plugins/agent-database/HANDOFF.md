...
> **Не зроблено:** прогін `@sql-mentor` на реалістичній задачі з цієї теми. За робочим процесом
> плагіна це наступний крок і найдешевший спосіб знайти прогалини — див. нижче.
...
- **Найперше: прогнати `@sql-mentor` на задачі з порції ClickHouse/Cassandra** (внесено 2026-07-19,
  гілка `feat/db`, але **без тестового прогону**). Придатні задачі: «спроєктувати схему подій під
  ClickHouse і пояснити вибір `ORDER BY`/партиціювання + стратегію вставки» і «змоделювати
  бронювання під Cassandra з розрахунком розміру партиції та бакетуванням». Другу варто довести до
  **📒 запису в банк задач** — там досі немає жодного кейсу з розрахунком `Nv`, а формула саме
  того й варта, щоб бути показаною на повному прикладі.
