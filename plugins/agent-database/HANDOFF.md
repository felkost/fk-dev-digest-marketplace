...
**Кураторські пропозиції чекають на схвалення людини** (файли не чіпав): 3 × «📌 Урок-кандидат»
(семантика `ReplacingMergeTree`; `Nullable`→`DEFAULT 0` змінює `AVG`, хоч і не змінює `SUM`;
бакет не можна робити дрібнішим за одиницю атомарної операції) і 2 × «📒 Запис у банк задач»
(ClickHouse-операційка — у банку немає жодного такого кейсу; моделювання Cassandra з розрахунком
партицій — теж немає жодного).
...
- **Найперше: закрити прогалини, які знайшов тестовий прогін порції ClickHouse/Cassandra** (список
  вище, § «Тестовий прогін `@sql-mentor`»). Пріоритет — **семантика `ReplacingMergeTree`** у
  `clickhouse.md` (движок згадується чотири рази, а описаний не був; це найпопулярніший вибір під
  подієві таблиці й джерело найтихіших багів) і **заборона TTL на `counter`** у `cassandra-cql.md`.
- **Розглянути 3 «📌 Урок-кандидат» і 2 «📒 Запис у банк задач»** з того ж прогону — чекають на
  схвалення. Обидві задачі з прогону вже написані повністю за шаблоном `task-bank.md`, тож внесення
  зводиться до вичитки й вставки. У банку досі **немає жодного** кейсу ні з ClickHouse-операційки,
  ні з моделювання Cassandra з розрахунком `Nv`.
