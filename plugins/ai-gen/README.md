...
├── explain-llm-internals        ← токенізація, увага, масштабування, сервінг, LoRA/QLoRA, reasoning-моделі, параметри генерації — рівень «чому»
...
- «Який temperature/top_p ставити», seed, presence/frequency penalty → `explain-llm-internals`
...
## API-ключі для робочих прикладів (build-ai-examples)
...
Приклад у [skills/build-ai-examples/scripts/mcp_example/](skills/build-ai-examples/scripts/mcp_example/)
потребує **OpenRouter** лише для `agent.py` (сам MCP-сервер і безкоштовна жива перевірка
`test_live_stdio.py` — без жодного ключа) — детально в
[mcp-example.md](skills/build-ai-examples/references/mcp-example.md#run-it).
The example in [skills/build-ai-examples/scripts/reflexion_example/](skills/build-ai-examples/scripts/reflexion_example/)
